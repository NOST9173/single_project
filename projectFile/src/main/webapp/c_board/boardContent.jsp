...
<form name="frm">
		<div>
			<table border="1" style="text-align:center; border-collapse: collapse; margin-left: auto; margin-right: auto; width:820px;">
				<tr>
					<td style="text-align: center;" border-right="1">제목</td>
					<td><%=bv.getSubject() %></td>
				</tr>
				<tr>
				...
				<%-- <tr>
					...
					<a href="<%=request.getContextPath()%>/c_board/fileDownload.do?filename=<%=bv.getFilename()%>"><%=bv.getFilename() %></a>
					</td>
				</tr> --%>
				<tr>
				...
				<tr>
					...
					<input type="button" class="button" name="modify" value="수정" onclick="location.href='<%=request.getContextPath()%>/c_board/boardModify.do?bidx=<%=bv.getBidx()%>'"> 
					<input type="button" class="button" name="delete" value="삭제" onclick="check()">
					<%} else { %><input type="hidden"><%} %>
					<input type="hidden" name="bidx" value="<%=bv.getBidx() %>"> 
					<input type="button" class="button" name="reply" value="답변" onclick="location.href='<%=request.getContextPath()%>/c_board/boardReply.do?bidx=<%=bv.getBidx()%>&originbidx=<%=bv.getOriginbidx()%>&depth=<%=bv.getDepth()%>&level_=<%=bv.getLevel_()%>'"> 
					<input type="button" class="button" name="list" value="목록" onclick="location.href='<%=request.getContextPath()%>/c_board/boardList.do'"> 
					</td>
				</tr>
				<tr>
