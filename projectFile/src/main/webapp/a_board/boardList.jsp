...
<form name="frm" action="<%=request.getContextPath() %>/a_board/boardList.do" method="post">
	<table border="1" style="text-align:center; border-collapse: collapse; margin-left: auto; margin-right: auto; width:900px; border:none; border-bottom:1px solid #CCC; border-right:0; border-left:0; ">
		...
		<tr style="height:30px;">
			...
			%>
			<a href="<%=request.getContextPath()%>/a_board/boardContent.do?bidx=<%=bv.getBidx() %>"><%=bv.getSubject() %></a></td>
			<td><%=bv.getWriter() %></td>
		...
		<tr>
			...
			<td colspan="5" style="text-align: right; border-bottom:0;">
				<a href="<%=request.getContextPath() %>/a_board/boardWrite.do"><input type="button" class="button" name="btn" value="글쓰기"></a>  
			</td>
		</tr>
	</table>
...
<table style="text-align:center; border-collapse: collapse; margin-left: auto; margin-right: auto;  width:800px;">
	<tr>
		...
		<% if (pm.isPrev()==true) {
			out.println("<a href='"+request.getContextPath()+"/a_board/boardList.do?page="+(pm.getStartPage()-1)+"&keyword="+pm.encoding(pm.getScri().getKeyword())+"&searchType="+pm.getScri().getSearchType()+"'>◀</a>");
		}
			%>
		</td>
		...
		<% 
			for(int i =pm.getStartPage();i<=pm.getEndPage();i++){
				out.println("<a href='"+request.getContextPath()+"/a_board/boardList.do?page="+i+"&keyword="+pm.encoding(pm.getScri().getKeyword())+"&searchType="+pm.getScri().getSearchType()+"'>"+i+"</a>");
		...
		<% if (pm.isNext() && pm.getEndPage() > 0){
			out.println("<a href='"+request.getContextPath()+"/a_board/boardList.do?page="+(pm.getEndPage()+1)+"&keyword="+pm.encoding(pm.getScri().getKeyword())+"&searchType="+pm.getScri().getSearchType()+"'>▶</a>");
		} %>
		</td>
	</tr>
</table>
