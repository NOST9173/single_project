...
function deletePost(){
	...
	if(confirmed){
		...
		form.action = "<%=request.getContextPath()%>/b_board/boardDeleteAction.do";	 
		form.submit();	// 폼값 전송
...
function check(){
  	...
  	if (confirm("삭제를 실행합니다. 삭제 하시겠습니까?")) {
  	    // 확인 버튼 클릭 시 동작
  		alert("삭제 되었습니다.");
  		fm.action = "<%=request.getContextPath()%>/b_board/boardDeleteAction.do";
		...
		<div>
			<table border="1" style="text-align:center; border-collapse: collapse; margin-left: auto; margin-right: auto; width:820px;">
				...
				<tr>
					...
					<input type="button" class="button" name="modify" value="수정" onclick="location.href='<%=request.getContextPath()%>/b_board/boardModify.do?bidx=<%=bv.getBidx()%>'"> 
					<input type="button" class="button" name="delete" value="삭제" onclick="check()">
					<%} else{ %><input type="hidden">
					<%} %>
					<input type="hidden" name="bidx" value="<%=bv.getBidx() %>"> 
					<input type="button" class="button" name="reply" value="답변" onclick="location.href='<%=request.getContextPath()%>/b_board/boardReply.do?bidx=<%=bv.getBidx()%>&originbidx=<%=bv.getOriginbidx()%>&depth=<%=bv.getDepth()%>&level_=<%=bv.getLevel_()%>'"> 
