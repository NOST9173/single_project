...
public class BoardDao2 {
	...
	public int insertBoard(String subject, String content, String writer, String ip, int midx) {
		...
		String sql = "INSERT INTO beomjun9173.bj_b_board(originbidx, depth, level_, subject, content, writer, ip, midx) "
				+ "VALUES ( 0, 0, 0, ?, ?, ?, ?, ?)";
...
//		String sql = "select * from a_board where delyn='N' order by originbidx desc, depth asc";
		/*
		String sql = "SELECT * FROM (SELECT ROWNUM AS rnum, A.* FROM (SELECT * FROM bj_b_board WHERE delyn='N' " + str
				+ " ORDER BY ORIGINbidx desc, depth ASC) A)B WHERE rnum BETWEEN ? AND ?";
		*/
		String sql = "SELECT * FROM beomjun9173.bj_b_board where delyn='N'"+str+"order by bidx desc, depth asc limit ?,?" ;
	...
	public BoardVo boardSelectOne(int bidx) {
		...
		String sql = "select * from beomjun9173.bj_b_board where bidx=?";
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bidx);
			rs = pstmt.executeQuery();
	...
	public int modifyBoard(String subject, String content, String writer, int bidx) {
		...
		// 8��
		String sql = "update beomjun9173.bj_b_board set subject=?, content=?, writer=?, writeday=datetime default now() where bidx=?";
	...
	public int deleteBoard(int bidx) {
		...
		// 8
		String sql = "update beomjun9173.bj_b_board set delyn='Y' where bidx=?";
	...
	public int replyBoard(BoardVo bv) {
		...
		// 8
		String sql1 = "update beomjun9173.bj_b_board set depth=depth+1 where originbidx=? and depth > ?";
		String sql2 = "insert into beomjun9173.bj_b_board(originbidx, depth, level_, subject, content, writer, ip, midx)"
				+ "VALUES (?,?,?,?,?,?,?,? )";
	...
	public int boardTotal(SearchCriteria scri) {
		...
		String sql = "select count(*) as cnt from beomjun9173.bj_b_board where delyn='N'" + str + "";
